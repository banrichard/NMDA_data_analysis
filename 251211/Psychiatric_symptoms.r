...
names(my.dat)[names(my.dat) == "Psychiatric symptoms"] <- "PsychSym"

# Check current class
class(my.dat$PsychSym)
table(my.dat$PsychSym)  # Should show 0 and 1

# Convert to factor (binary logistic regression expects factor or 0/1)
my.dat$PsychSym <- factor(my.dat$PsychSym, levels = c(0, 1), ordered = FALSE)
...
for (pollutant in pollutants) {
  ...
  for (lag_col in lag_cols) {
    ...
    model <- glm(PsychSym ~ ., data = my.dat[, c("PsychSym", lag_col)], 
...
# ggsave("NMDAR_PsychiatricSymptoms.png", p_combined, width = 12, height = 15, dpi = 300, units = "in")
