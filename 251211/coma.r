...
# Note: We don't need 'ordinal' for binary outcome, but keeping for compatibility

# ============================
# 2. Simplify Outcome Variable Name & Convert to Factor
# ============================
# Rename column for simplicity
names(my.dat)[names(my.dat) == "Coma"] <- "Coma"  # Already simple, but ensure it exists

# Check current class
class(my.dat$Coma)
table(my.dat$Coma)  # Should show 0 and 1

# Convert to factor (binary logistic regression expects factor or 0/1)
my.dat$Coma <- factor(my.dat$Coma, levels = c(0, 1), ordered = FALSE)
...
for (pollutant in pollutants) {
  ...
  for (lag_col in lag_cols) {
    ...
    model <- glm(Coma ~ ., data = my.dat[, c("Coma", lag_col)], 
...
#   plot_annotation(title = "Impact of Pollutants on Coma",
...
# ggsave("NMDAR_Coma_Ordinal_Regression_NoText.png", p_combined, width = 12, height = 15, dpi = 300, units = "in")
...
p_combined <- plot_list[[1]] + plot_list[[2]] + plot_list[[3]] + plot_list[[5]] + 
  plot_list[[6]] + plot_list[[7]] + plot_list[[4]] +
  plot_annotation(title = "Impact of Air Pollutants on Coma",
                  theme = theme(plot.title = element_text(hjust = 0.5, size = 17, color = "black")))
...
# Optional: Save as PNG
ggsave("NMDAR_Coma.png", p_combined, width = 12, height = 15, dpi = 300, units = "in")
...
write_xlsx(table_df, "NMDAR_Coma_Results_Table.xlsx")
