...
# ============================
# 2. Simplify Outcome Variable Name & Recode to Binary (0/1)
# ============================
# Rename column for simplicity
names(my.dat)[names(my.dat) == "Brainstem dysfunction（0=none; 1=gaze paresis; 2=tube feeding; 3=ventilator care due to hypoventilation）"] <- "Brainstem"

# Check current class
class(my.dat$Brainstem)
table(my.dat$Brainstem)  # Should show 0,1,2,3

# Recode: 0 → 0, 1/2/3 → 1
my.dat$Brainstem <- ifelse(as.numeric(as.character(my.dat$Brainstem)) == 0, 0, 1)

# Convert to factor (binary logistic regression expects factor or 0/1)
my.dat$Brainstem <- factor(my.dat$Brainstem, levels = c(0,1), ordered = FALSE)

# Verify
str(my.dat$Brainstem)
table(my.dat$Brainstem)  # Should show only 0 and 1
...
# ============================
# 4. Fit Binary Logistic Regression for Each Pollutant-Lag
# ============================
results_list <- list()
...
for (pollutant in pollutants) {
  ...
  for (lag_col in lag_cols) {
    ...
    model <- glm(Brainstem ~ ., data = my.dat[, c("Brainstem", lag_col)], 
...
# # ============================
# # 6. Create 7 Individual Plots (One per Pollutant) — NO TEXT LABELS
# # ============================
...
# 
# # ============================
# # 7. Combine All 7 Plots in a Single Row — CO LAST
# # ============================
# p_combined <- plot_list[[1]] + plot_list[[2]] + plot_list[[3]] + plot_list[[5]] + 
#   plot_list[[6]] + plot_list[[7]] + plot_list[[4]] +
#   plot_annotation(title = "Impact of Pollutants on Brainstem Dysfunction",
...
# ggsave("NMDAR_Brainstem_Binary_Results.png", p_combined, width = 12, height = 15, dpi = 300, units = "in")
...
p_combined <- plot_list[[1]] + plot_list[[2]] + plot_list[[3]] + plot_list[[5]] + 
  plot_list[[6]] + plot_list[[7]] + plot_list[[4]] +
  plot_annotation(title = "Impact of Air Pollutants on Brainstem Dysfunction",
                  theme = theme(plot.title = element_text(hjust = 0.5, size = 17, color = "black")))
...
write_xlsx(table_df, "NMDAR_Brainstem_Binary_Results_Table.xlsx")
