...
names(my.dat)[names(my.dat) == "Memory dysfunction"] <- "MemoryDys"

# Check current class
class(my.dat$MemoryDys)
table(my.dat$MemoryDys)  # Should show 0 and 1

# Convert to factor (binary logistic regression expects factor or 0/1)
my.dat$MemoryDys <- factor(my.dat$MemoryDys, levels = c(0, 1), ordered = FALSE)

# Verify
str(my.dat$MemoryDys)
...
for (pollutant in pollutants) {
  ...
  for (lag_col in lag_cols) {
    ...
    model <- glm(MemoryDys ~ ., data = my.dat[, c("MemoryDys", lag_col)], 
...
#   plot_annotation(title = "Impact of Pollutants on Memory Dysfunction",
...
# ggsave("NMDAR_MemoryDysfunction.png", p_combined, width = 12, height = 15, dpi = 300, units = "in")
...
p_combined <- plot_list[[1]] + plot_list[[2]] + plot_list[[3]] + plot_list[[5]] + 
  plot_list[[6]] + plot_list[[7]] + plot_list[[4]] +
  plot_annotation(title = "Impact of Air Pollutants on Memory Dysfunction",
                  theme = theme(plot.title = element_text(hjust = 0.5, size = 17, color = "black")))
...
write_xlsx(table_df, "NMDAR_MemoryDysfunction.xlsx")
