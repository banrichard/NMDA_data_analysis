...
# ============================
# Rename column for simplicity (if needed)
names(my.dat)[names(my.dat) == "Dyskinesia/dystonia"] <- "Dyskinesia"

# Check current class
class(my.dat$Dyskinesia)
table(my.dat$Dyskinesia)  # Should show 0 and 1

# Convert to factor (binary logistic regression expects factor or 0/1)
my.dat$Dyskinesia <- factor(my.dat$Dyskinesia, levels = c(0, 1), ordered = FALSE)

# Verify
str(my.dat$Dyskinesia)
...
for (pollutant in pollutants) {
  ...
  for (lag_col in lag_cols) {
    ...
    model <- glm(Dyskinesia ~ ., data = my.dat[, c("Dyskinesia", lag_col)], 
...
#   plot_annotation(title = "Impact of Pollutants on Dyskinesia/Dystonia",
...
# ggsave("NMDAR_Dyskinesia.png", p_combined, width = 12, height = 15, dpi = 300, units = "in")
...
p_combined <- plot_list[[1]] + plot_list[[2]] + plot_list[[3]] + plot_list[[5]] + 
  plot_list[[6]] + plot_list[[7]] + plot_list[[4]] +
  plot_annotation(title = "Impact of Air Pollutants on Dyskinesia/Dystonia",
                  theme = theme(plot.title = element_text(hjust = 0.5, size = 17, color = "black")))
...
write_xlsx(table_df, "NMDAR_Dyskinesia.xlsx")
