...
for (p in pollutant_list) {
  ...
  outcome <- "as.factor(CSFrank)"  # assuming mRS is your severity score
...
output_folder <- "./report_result_prediction_csf_v3"
...
for (pollutant in names(model_list)) {
  ...
  true <- reg.dat.r$CSFrank  # assuming mRS is your severity outcome
  
  # Step 4: Compute metrics
  accuracy <- mean(as.numeric(pred_class) == as.numeric(true))
...
# Save as HTML file
file_path_html <- file.path(output_folder, "prediction_performance_table_csf.html")
