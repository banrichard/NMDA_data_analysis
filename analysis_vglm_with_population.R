...
pollutants_to_analyze <- get_pollutant_list(
  data = my.dat,
  marker_column = "Date of onset1"
)

# 4. 创建结果文件夹
dir.create("result_figure_vglm_with_offset", showWarnings = FALSE)
dir.create("report_result_vglm_with_offset", showWarnings = FALSE)


# =================================================================
# 核心循环：为每个污染物执行聚合、建模、绘图、输出
# =================================================================
...
for (pollutant in pollutants_to_analyze) {
  ...
  # --- a. 【新】数据准备：直接对原始数据进行 count 操作 ---
  #    这会生成一个只包含 n > 0 的稀疏数据集
  ...
  # --- b. 运行 vglm 模型 ---
  vglm_results <- run_vglm_analysis(
    reg_data = reg_dat,
  ...
  # 计算 z 值，其含义是“给定有病例发生，预测的平均病例数”
  z_matrix <- outer(x_coords, y_coords, function(x, y) exp(estbeta[(y + 1), 1] + x * estbeta[(y + 1), 2]))
  
  # --- f. 绘图并保存 ---
  output_filename_jpeg <- file.path("result_figure_vglm_with_offset", paste0(pollutant, "_month.jpeg"))
  ...
  # --- g. 生成并保存 HTML 表格 ---
  output_filename_html <- file.path("report_result_vglm_with_offset", paste0(pollutant, "_summary.html"))
  analysis_table <- kable(summary_with_stars, digits = 4, align = "c", 
                          caption = paste0("Summary of ", pollutant, " Results (Zero-Truncated)"), format = "html")
