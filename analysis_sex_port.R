...
dir.create("adjusted_glm_plots_age_ratio_only", showWarnings = FALSE)
dir.create("adjusted_glm_reports_age_ratio_only", showWarnings = FALSE)
...
# 定义您想在模型中调整的协变量
covariates_to_include <- c("sex_ratio_MvsW")
for (pollutant in pollutants_to_analyze) {
  ...
  # f. 绘图并保存
  output_filename_jpeg <- file.path("adjusted_glm_plots_age_ratio_only", paste0(pollutant, "_month.jpeg"))
  ...
  # g. 生成并保存 HTML 表格
  output_filename_html <- file.path("adjusted_glm_reports_age_ratio_only", paste0(pollutant, "_summary.html"))
